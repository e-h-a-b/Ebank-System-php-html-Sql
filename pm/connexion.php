...
else
{
	...
	if(isset($_POST['usn'], $_POST['pass']))
	{
		//We remove slashes depending on the configuration
		if(get_magic_quotes_gpc())
		{
			$ousername = stripslashes($_POST['usn']);
			$username = mysql_real_escape_string(stripslashes($_POST['usn']));
		...
		$req = mysql_query('select userPass,userId from users where userName="'.$username.'"');
		$dn = mysql_fetch_array($req);
		//We compare the submited password and the real one, and we check if the user exists
		if($dn['userPass']==$password and mysql_num_rows($req)>0)
		{
...
<div class="content">
    <form action="connexion.php" method="post">
        ...
        <div class="center">
            <label for="usn">Username</label><input type="text" name="usn" id="usn" value="<?php echo htmlentities($ousername, ENT_QUOTES, 'UTF-8'); ?>" /><br />
